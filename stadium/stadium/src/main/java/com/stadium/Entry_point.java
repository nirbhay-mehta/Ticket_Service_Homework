...
public class Entry_point  {
	...
	public static void main(String[] args) {
		...
		int useCaseNo = 4;
		do {
			System.out.println("Select from the below usecase:");
			System.out.println("Press 1 --> To find the no of available seats");
			System.out.println("Press 2 --> To hold the best available seat for yourelf!");
			System.out.println("Press 3 --> To reserve the seats");
	...
	public void findAvailableSeats() {
		System.out.println("Select the level number where you want the seats");
	...
	/*public void selectVenuelevel(int levelId, int rows, int seats, String name) {
		...
		System.out.println("Seat No is" + seats + "Name" + name);
	}*/
	...
	public ArrayList<Integer> findAndHoldSeats(int hold_ticket, int venueLevel, String customer_email) {
		...
		/*SeatHold hold_seatid = new SeatHold();
		if (hold_seatid.seatStatus == SeatStatusType.AVAILABLE) {
			hold_seatid.seatStatus = SeatStatusType.ONHOLD;
		} else {
			checkholdtickets(hold_ticket);
		}
		return seatsIds;
	}
// ******************************
	void checkholdtickets(int hold_ticket)
	{
		SeatHold hold_seatid = new SeatHold();
		// if trying to hold the reserrved seats
		if (hold_seatid.seatStatus == SeatStatusType.RESERVED) {
			System.out.println("Sorry it is already being boooked");
		} 
		// if the seats is available making it hold
		else if (hold_seatid.seatStatus == SeatStatusType.ONHOLD) {
...
// ************************************
	...
	/// making hold seats to available for user if the seatis not reserved is greater within 60seconds
				...
				for (Rows row : ar) {
					...
					for (SeatHold seat : s) {
						if ((seat.seatStatus == SeatStatusType.ONHOLD)
								&& (Math.abs(d.getTime() - seat.getOnHoldTime())) > 60000) {
							seat.seatStatus = SeatStatusType.AVAILABLE;
	...
	// if the user has hold the seat then only the user can reserve the seats since when user holds the seat seatid is given
